Snips.addText("de", :dunno, "Ich weiß nicht was ich machen soll")
Snips.addText("de", :no_sounds, "Ich kann keinen Weckerton in der config-Datei finden!")
Snips.addText("de", :no_sound_file, "Die Datei mit dem Klingelton existiert nicht")
Snips.addText("de", :which_sound, "Ich habe nicht verstanden, welchen Klingelton ich einstellen soll")
Snips.addText("de", :sound_set, "Der Klingelton ist neu eingestellt und ist jetzt")
Snips.addText("de", :with, "mit")

Snips.addText("de", :wakeup_scheduled, "Der Wecker ist eingestellt auf")
Snips.addText("de", :deleted, "Der Wecker ist ausgeschaltet!")

Snips.addText("de", :good_morning, "Guten morgen, es ist")
Snips.addText("de", :good_morning, "Guten morgen, heute ist")
Snips.addText("de", :good_morning, "Guten morgen, am")

Snips.addText("de", :hour, "Stunde")
Snips.addText("de", :hours, "Stunden")
Snips.addText("de", :minute, "Minute")
Snips.addText("de", :minutes, "Minuten")
Snips.addText("de", :and, "und")

Snips.addText("en", :dunno, "I don't know what to do")
Snips.addText("en", :no_sounds, "I am not able to find any ringtones in the config file!")
Snips.addText("en", :no_sound_file, "I'm unable to find the file with the requested ringtone")
Snips.addText("en", :which_sound, "I don't know which ringtone to use")
Snips.addText("en", :sound_set, "The new ringtone is")
Snips.addText("en", :with, "with")

Snips.addText("en", :wakeup_scheduled, "The wakeup call ist scheduled for")
Snips.addText("en", :deleted, "The wakeup call is deleted!")


Snips.addText("en", :good_morning, "Good morning, it is ")
Snips.addText("en", :good_morning, "Good morning, today is")

Snips.addText("en", :hour, "hour")
Snips.addText("en", :hours, "hours")
Snips.addText("en", :minute, "minute")
Snips.addText("en", :minutes, "minutes")
Snips.addText("en", :and, "and")



# regex for time correction with input string:
#
Snips.addText("de", :tomorrow, "morgen")
Snips.addText("en", :tomorrow, "tomorrow")
